...
function patch(csrfToken, type, id, data) {
  jQuery.ajax({
    url: `${baseUrl}/${type}/${id}?_format=hal_json`,
    method: 'PATCH',
    headers: {
...
function updateApplication(nodeId, completed, canceled) {
  ...
  getCsrfToken(function (csrfToken) {
    patch(csrfToken, 'node', nodeId, nodeData);
  }); 
}
...
function updateForumSubscription(forumId, userId, subscribe) {
  var userData = {
    ...
    field_forum_subscriptions: {
      value: subscribe
    },
  };
    
  getCsrfToken(function (csrfToken) {
    patch(csrfToken, 'user', userId, userData);
  });
}
